...
int check_input_format() {
  ...
  // Inspired by online resource:
https://stackoverflow.com/questions/12142174/run-a-python-script-with-arguments
  // Check my Wiki Page:
https://github.com/se-thermalanalytics/DC-DoE-CU-LBL-SE/wiki/Embed-python-script-in-C-codes
  FILE* file;
  int argc;
  char * argv[1];
...
int read_sci_max(PARA_DATA *para, REAL **var) {
  ...
  if (ifDouble) {
    sscanf(string, "%lf %lf %lf", &para->geom->Lx, &para->geom->Ly,
           &para->geom->Lz);
  }
  else {
    sscanf(string, "%f %f %f", &para->geom->Lx, &para->geom->Ly,
           &para->geom->Lz);
  }
...
///\return 0 if no error occurred
///////////////////////////////////////////////////////////////////////////////
// FIXME: THIS FUNCTION IS TOO LENGTHT AND DIFFICULT TO READ AND
// MAINTAIN---WEITIAN
int read_sci_input(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  *****************************************************************************/
  ...
  if (ifDouble) {
    for (i = 1; i <= imax; i++)
      fscanf(file_params, "%lf", &delx[i]);
    fscanf(file_params, "\n");
    for (j = 1; j <= jmax; j++)
      fscanf(file_params, "%lf", &dely[j]);
    fscanf(file_params, "\n");
    for (k = 1; k <= kmax; k++)
      fscanf(file_params, "%lf", &delz[k]);
    fscanf(file_params, "\n");
  }
  else {
    for (i = 1; i <= imax; i++)
  ...
  if (para->bc->nb_inlet != 0) {
    ...
    for (i = 0; i < para->bc->nb_inlet; i++) {
      ...
      if (para->bc->inletName[i] == NULL) {
        sprintf(msg,
                "read_sci_input(): Could not allocate memory for "
                "para->bc->inletName[%d].",
                i);
        ffd_log(msg, FFD_ERROR);
        return 1;
      ...
      sprintf(msg, "read_sci_input(): para->bc->inletName[%d]=%s", i,
              para->bc->inletName[i]);
      ffd_log(msg, FFD_NORMAL);
      /*.......................................................................
      ...
      fgets(string, 400, file_params);
      if (ifDouble) {
        sscanf(string, "%d%d%d%d%d%d%lf%lf%lf%lf%lf", &SI, &SJ, &SK, &EI, &EJ,
               &EK, &TMP, &MASS, &U, &V, &W);
      }
      else {
      ...
      sprintf(msg, "read_sci_input(): VX=%f, VY=%f, VZ=%f, T=%f, Xi=%f", U, V,
              W, TMP, MASS);
      ffd_log(msg, FFD_NORMAL);
        ...
        for (ii = SI; ii <= EI; ii++) {
          for (ij = SJ; ij <= EJ; ij++) {
            for (ik = SK; ik <= EK; ik++) {
              if (SI == EI) {
                momentum_kick =
                    get_momentum_kick(fabs(U), beta_tmp, h_momentum);
                var[VXS][IX(ii + 1, ij, ik)] = momentum_kick;
              }
              else if (SJ == EJ) {
                momentum_kick =
                    get_momentum_kick(fabs(V), beta_tmp, h_momentum);
                var[VYS][IX(ii, ij + 1, ik)] = momentum_kick;
              }
              else if (SK == EK) {
                momentum_kick =
                    get_momentum_kick(fabs(W), beta_tmp, h_momentum);
                var[VZS][IX(ii, ij, ik + 1)] = momentum_kick;
              }
            }
          }
        }
  ...
  *****************************************************************************/
  ...
  if (para->bc->nb_outlet != 0) {
    para->bc->outletName =
        (char **)malloc(para->bc->nb_outlet * sizeof(char *));
    ...
    for (i = 0; i < para->bc->nb_outlet; i++) {
      ...
      if (para->bc->outletName[i] == NULL) {
        sprintf(msg,
                "read_sci_input(): Could not allocate memory "
                "for para->bc->outletName[%d].",
                i);
        ffd_log(msg, FFD_ERROR);
        return 1;
      }
      ...
      sprintf(msg, "read_sci_input(): para->bc->outletName[%d]=%s", i,
              para->bc->outletName[i]);
      ffd_log(msg, FFD_NORMAL);
      ...
      sprintf(msg, "read_sci_input(): VX=%f, VY=%f, VX=%f, T=%f, Xi=%f", U, V,
              W, TMP, MASS);
      ffd_log(msg, FFD_NORMAL);
          ...
          for (ik = SK; ik <= EK; ik++) {
            ...
            if (flagp[IX(ii, ij, ik)] < 0) {
              ...
              if (tile_or_outlet == TILE) {
                // read name
                if (ifDouble) {
                  sscanf(para->bc->outletName[i], "%s%lf", tile_name_tmp,
                         &var[TILE_OPEN_BC][IX(ii, ij, ik)]);
                }
                else {
                  sscanf(para->bc->outletName[i], "%s%f", tile_name_tmp,
                         &var[TILE_OPEN_BC][IX(ii, ij, ik)]);
                }

                // read the resistance parameter
                tile_opening = var[TILE_OPEN_BC][IX(ii, ij, ik)];
                // printf("is opening is %f\n", var[TILE_OPEN_BC][IX(ii, ij,
                // ik)]);

                // Calculate the resistance based on the paper
                // @inproceedings{vangilder2015development,
                //		title = { Development of a Raised - Floor Plenum
                // Design Tool }, 		author = { VanGilder, James W
                // and Zhang, Xuanhang Simon }, 		year = { 2015 },
                // organization = { American Society of Mechanical Engineers }
                //		}
                // SHOULD MULTIPLE BY A 0.5 AT THE END, ACCORDINTG TO THE
                // EQUATIONS.
                var[TILE_RESI_BC][IX(ii, ij, ik)] =
                    1 / pow(tile_opening, 2) *
                    (1.0 + 0.5 * pow(1 - tile_opening, 0.75) +
                     1.414 * pow(1 - tile_opening, 0.375)) *
                    0.5;
              }
            }
  ...
  *****************************************************************************/
  ...
  if (para->bc->nb_port > 0) {
    ...
    if (para->bc->portName == NULL) {
      ffd_log(
          "read_sci_input(): Could not allocate memory for para->bc->portName.",
          FFD_ERROR);
      return 1;
    }
    /*--------------------------------------------------------------------------
    ...
    for (i = 0; i < para->bc->nb_inlet; i++) {
      ...
      sprintf(msg, "read_sci_input(): Port[%d]:%s", i, para->bc->portName[i]);
      ffd_log(msg, FFD_NORMAL);
    }
    ...
    for (i = 0; i < para->bc->nb_outlet; i++) {
      ...
      if (para->bc->portName[i + j] == NULL) {
        ...
        sprintf(msg, "read_sci_input(): Port[%d]:%s", i + j,
                para->bc->portName[i + j]);
        ffd_log(msg, FFD_NORMAL);
      }
    }
  ...
  *****************************************************************************/
  ...
  // allocate memory for cooling airflow direction for racks, and mapping of
  // outlet to inlet cells in rack
  if (para->bc->nb_rack != 0) {
    ...
    if (para->bc->rackName == NULL) {
      ffd_log(
          "read_sci_input(): Could not allocate memory for para->bc->rackName.",
          FFD_ERROR);
      return 1;
    }
    ...
    if (para->bc->HeatDiss == NULL) {
      ffd_log(
          "read_sci_input(): Could not allocate memory for para->bc->HeatDiss.",
          FFD_ERROR);
      return 1;
    }
    ...
    if (para->bc->RackArea == NULL) {
      ffd_log(
          "read_sci_input(): Could not allocate memory for para->bc->RackArea.",
          FFD_ERROR);
      return 1;
    }
    ...
    if (para->bc->RackDir == NULL) {
      ffd_log(
          "read_sci_input(): Could not allocate memory for para->bc->RackDir.",
          FFD_ERROR);
      return 1;
    }
    ...
    // allocate memory for mapping of outlet cells to the inlets cells for each
    // rack
    para->bc->RackMap = (int **)malloc(para->bc->nb_rack * sizeof(int *));
    if (para->bc->RackMap == NULL) {
      ffd_log(
          "read_sci_input(): Could not allocate memory for para->bc->RackMap.",
          FFD_ERROR);
      return 1;
    }
    for (i = 0; i < para->bc->nb_rack; i++) {
    ...
    para->sens->coordMoniPoints =
        (REAL ***)malloc(para->bc->nb_rack * sizeof(REAL **));
    ...
    for (i = 0; i < para->bc->nb_rack; i++) {
      para->sens->coordMoniPoints[i] =
          (REAL **)malloc(RACK_MONITOR_POINTS * sizeof(REAL *));
    ...
    para->sens->indexMoniPoints =
        (int ***)malloc(para->bc->nb_rack * sizeof(int **));
    ...
    // initialize coordinates of the monitoring points in front racks
    for (i = 0; i < para->bc->nb_rack; i++) {
      para->sens->indexMoniPoints[i] =
          (int **)malloc(RACK_MONITOR_POINTS * sizeof(int *));
  ...
  if (para->bc->nb_block != 0) {
    ...
    for (i = 0; i < para->bc->nb_block; i++) {
      ...
      sprintf(msg, "read_sci_input(): para->bc->blockName[%d]=%s", i,
              para->bc->blockName[i]);
      ffd_log(msg, FFD_NORMAL);
      ...
      if (ifDouble) {
        ...
        sscanf(string, "%d%d%d%d%d%d%d%f", &SI, &SJ, &SK, &EI, &EJ, &EK, &FLTMP,
               &TMP);
      }
      ...
      // read the plastic curtain or ceiling tile (initially implemented by Wei
      // Tian, 9/13/2018)
      // FIXME: only one cell in the width direction is allowed, otherwise no
      // resistance can be added
      if ((strstr(para->bc->blockName[i], "Curtain") != NULL) ||
          (strstr(para->bc->blockName[i], "CeilingTile") != NULL)) {
        // read name
        if (ifDouble) {
          sscanf(para->bc->blockName[i], "%s%lf", curtain_name_tmp,
                 &curtain_open_ratio);
        }
        else {
          sscanf(para->bc->blockName[i], "%s%f", curtain_name_tmp,
                 &curtain_open_ratio);
        }
        ...
        for (ii = SI; ii <= EI; ii++) {
          for (ij = SJ; ij <= EJ; ij++) {
            for (ik = SK; ik <= EK; ik++) {
              ...
              if (SI == EI) {
                var[RESX][IX(ii, ij, ik)] =
                    1 / pow(curtain_open_ratio, 2) *
                    (1.0 + 0.5 * pow(1 - curtain_open_ratio, 0.75) +
                     1.414 * pow(1 - curtain_open_ratio, 0.375)) *
                    0.5;
              }
              else if (SJ == EJ) {
                var[RESY][IX(ii, ij, ik)] =
                    1 / pow(curtain_open_ratio, 2) *
                    (1.0 + 0.5 * pow(1 - curtain_open_ratio, 0.75) +
                     1.414 * pow(1 - curtain_open_ratio, 0.375)) *
                    0.5;
      ...
      // set the boundary conditions based on that if it is regular block or a
      // rack if it is a rack
      else if (strstr(para->bc->blockName[i], "Rack") != NULL) {
        // split the name information
        if (ifDouble) {
          sscanf(para->bc->blockName[i], "%s%d%lf%lf", rack_name_tmp,
                 &para->bc->RackDir[id_rack], &para->bc->HeatDiss[id_rack],
                 &para->bc->RackFlowRate[id_rack]);
        }
        else {
          sscanf(para->bc->blockName[i], "%s%d%f%f", rack_name_tmp,
                 &para->bc->RackDir[id_rack], &para->bc->HeatDiss[id_rack],
                 &para->bc->RackFlowRate[id_rack]);
        }
        ...
        // Store the cell information and boundary condition by looping all rack
        // cells
        for (ii = SI; ii <= EI; ii++) {
          for (ij = SJ; ij <= EJ; ij++) {
            for (ik = SK; ik <= EK; ik++) {
              ...
              if (para->bc->RackDir[id_rack] == 1) {
                if (ii == SI && ik != 0) {
                  ...
                  flagp[IX(ii, ij, ik)] =
                      SOLID;  // Flag for cells in the middle
                }
              }
        ...
        if (get_monitor_points(para, var, SI, SJ, SK, EI, EJ, EK,
                               para->bc->RackDir[id_rack], id_rack) != 0) {
          sprintf(msg,
                  "read_sci_input(): cannot find the monitoring points at the "
                  "front of the RACK %d",
                  id_rack + 1);
          ffd_log(msg, FFD_ERROR);
        }
      ...
      /*********************START OF CHANGE THE SCALE OF HOLA CELLS IN THE
       * MIDDLE*********************************************/
      /********************* FIX BY WEI TIAN,
       * 11/29/2018**********************************************************************/
      // this is a temporal fix for the problem when I found that FloVENT and
      // FFD have different results at the cells adjacent to the solid. This fix
      // is symmetrical for all the three directions west

      ii = SI;
      for (ij = SJ; ij <= EJ; ij++) {
        for (ik = SK; ik <= EK; ik++) {
          if (ii >= 1)
            var[X][IX(ii, ij, ik)] = var[GX][IX(ii - 1, ij, ik)];
        }
      ...
      for (ij = SJ; ij <= EJ; ij++) {
        for (ik = SK; ik <= EK; ik++) {
          ...
          var[X][IX(ii, ij, ik)] = var[GX][IX(ii, ij, ik)];
        }
      }  // end of east
      // south
      ij = SJ;
      for (ii = SI; ii <= EI; ii++) {
        for (ik = SK; ik <= EK; ik++) {
          ...
          if (ij >= 1)
            var[Y][IX(ii, ij, ik)] = var[GY][IX(ii, ij - 1, ik)];
        }
      }
      // north
      ij = EJ;
      for (ii = SI; ii <= EI; ii++) {
        for (ik = SK; ik <= EK; ik++) {
          ...
          var[Y][IX(ii, ij, ik)] = var[GY][IX(ii, ij, ik)];
        }
      }
      // floor
      ik = SK;
      for (ii = SI; ii <= EI; ii++) {
        for (ij = SJ; ij <= EJ; ij++) {
          ...
          if (ik >= 1)
            var[Z][IX(ii, ij, ik)] = var[GZ][IX(ii, ij, ik - 1)];
        }
      }
      // ceiling
      ik = EK;
      for (ii = SI; ii <= EI; ii++) {
        for (ij = SJ; ij <= EJ; ij++) {
          ...
          var[Z][IX(ii, ij, ik)] = var[GZ][IX(ii, ij, ik)];
        }
      }
      /*
       ii = SI;
       ...
       */
      /*********************END OF CHANGE THE SCALE OF HOLA CELLS IN THE
       * MIDDLE*********************************************/
  ...
  *****************************************************************************/
  ...
  if (para->bc->nb_wall != 0) {
    ...
    for (i = 0; i < para->bc->nb_wall; i++) {
      ...
      sprintf(msg, "read_sci_input(): para->bc->wallName[%d]=\"%s\"", i,
              para->bc->wallName[i]);
      ffd_log(msg, FFD_NORMAL);
      /*.......................................................................
      | Get the boundary conditions
  ...
  *****************************************************************************/
  ...
  if (para->bc->nb_source != 0) {
    if (ifDouble) {
      sscanf(string, "%s%d%d%d%d%d%d%lf", name, &SI, &SJ, &SK, &EI, &EJ, &EK,
             &MASS);
    }
    else {
...
void mark_cell(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  if (flagp[IX(i, j, k)] == SOLID) {
    ...
    if (i != 0)
      flagu[IX(i - 1, j, k)] = SOLID;
    if (j != 0)
      flagv[IX(i, j - 1, k)] = SOLID;
    if (k != 0)
      flagw[IX(i, j, k - 1)] = SOLID;
  }  // Mark solids
