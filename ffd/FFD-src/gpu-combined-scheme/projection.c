...
int project(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  ****************************************************************************/
  ...
  if (flagu[IX(i, j, k)] >= 0)
    continue;
  u[IX(i, j, k)] -= dt / rho * (p[IX(i + 1, j, k)] - p[IX(i, j, k)]) /
  ...
  if (flagv[IX(i, j, k)] >= 0)
    continue;
  v[IX(i, j, k)] -= dt / rho * (p[IX(i, j + 1, k)] - p[IX(i, j, k)]) /
  ...
  if (flagw[IX(i, j, k)] >= 0)
    continue;
  w[IX(i, j, k)] -= dt / rho * (p[IX(i, j, k + 1)] - p[IX(i, j, k)]) /
...
int check_mass_imbalance(PARA_DATA *para, REAL **var) {
  ...
  if (flagp[IX(i, j, k)] >= 0)
    continue;
  imbalance = (u[IX(i, j, k)] - u[IX(i - 1, j, k)]) /
