...
///\return Turbulent Kinematic viscosity
///
///\ This is to reimplement the turbulent thermal diffusivity for FFD using
///Chen's
///  zero equation model. Theoretically, by diving the turbulent viscosity over
///  the Pr number, we can easily get the turbulent themal diffusivity
///  coefficients. However, according to our test, that could cause severe
///  energy imblance for the whole space. The root reason to explain that is not
///  fully identified yet. However, we presumbaly beleive that the dradmatically
///  (over 100) spatial differnce of the thermal diffusivity coefficiet may lead
///  to energy imbalance, happening in the diffusion term. We also tried using
///  constant turbulence model, which essentially multiply 100 to the lamninar
///  one. Applying that in the FFD, we did not see the deterioration of energy
///  balance, at any level.
///\ Inspire by this, we propose to uniformly disseminate the turbulent thermal
///diffusivity over the
///  whole fluid space. According to our test, this method can perfectly
///  achieves energy balance for the room space, while keeping the turbulent
///  features in some extent of the thermal environment.
///
...
REAL alpha_t_chen_zero_equ(PARA_DATA *para, REAL **var) {
  ...
  if (flagp[IX(i, j, k)] >= 0)
    continue;
  sum_tur_alpha +=
      nu_t_chen_zero_equ(para, var, i, j, k) / 0.900000 /*Pr number*/;
  count += 1;
  END_FOR
