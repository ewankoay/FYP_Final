...
///
///\return 0 if no error occurred
///\ V1.0: 9/16/2017, by Wei Tian: first implementation by adding the boundary
/// condition for tiles \ V2.0: 1/22/2018, by Wei Tian: add the boundary
/// conditions for rack inlet and outlet
///////////////////////////////////////////////////////////////////////////////
int set_bnd_vel(PARA_DATA *para, REAL **var, int var_type, REAL *psi,
  ...
  case VZ:
    for (it = 0; it < index; it++) {
      ...
      }  // end of if(flagp[IX(i, j, k)] == OUTLET || flagp[IX(i, j, k)] ==
         // TILE)
      // Rack inlet, which is outlet of DC room
      // Apply a fixed velocity
      if (flagp[IX(i, j, k)] == RACK_INLET) {
...
int set_bnd_temp(PARA_DATA *para, REAL **var, int var_type, REAL *psi,
  ...
  int IMAX = imax + 2, IJMAX = (imax + 2) * (jmax + 2);
  REAL *aw = var[AW], *ae = var[AE], *as = var[AS], *an = var[AN];
  REAL *af = var[AF], *ab = var[AB], *b = var[B], *qflux = var[QFLUX],
       *qfluxbc = var[QFLUXBC];
  ...
  ****************************************************************************/
  for (it = 0; it < index; it++) {
    ...
    -------------------------------------------------------------------------*/
    if (flagp[IX(i, j, k)] == INLET || flagp[IX(i, j, k)] == RACK_OUTLET)
      ...
      if (BINDEX[3][it] == 0) {
        ...
        if (i == 0) {
          if (flagp[IX(i + 1, j, k)] == FLUID) {
            ...
            h = h_coef(para, var, i + 1, j, k, D);
            b[IX(i + 1, j, k)] +=
                qfluxbc[IX(i, j, k)] * ayz / (para->prob->rho * para->prob->Cp);
        ...
        if (j == 0) {
          ...
          h = h_coef(para, var, i, j + 1, k, D);
          b[IX(i, j + 1, k)] +=
              qfluxbc[IX(i, j, k)] * azx / (para->prob->rho * para->prob->Cp);
          // get the temperature of solid surface
        ...
        if (k == 0) {
          if (flagp[IX(i, j, k + 1)] == FLUID) {
            ...
            h = h_coef(para, var, i, j, k + 1, D);
            b[IX(i, j, k + 1)] +=
                qfluxbc[IX(i, j, k)] * axy / (para->prob->rho * para->prob->Cp);
...
///
///\return 0 if no error occurred
///\ V2.0: 9/16/2017, by Wei Tian: first implementation by adding the pressure
/// bc for tile
///////////////////////////////////////////////////////////////////////////////
...
///
///\for turbulent flows (with either cosntant or Chen's turbulence model), use
/// similarity
///  to derive the turbulen heat transfer coefficient based on laminar one,
///  calculated using a energy balance equation. For detail, see
///  heat_transfer_coefficeint.pdf file.
///
///\it is consistent with the recommendation in Chen and Xu's paper:
/// Chen, Qingyan, and Weiran Xu. "A zero-equation turbulence model for indoor
/// airflow simulation. " Energy and buildings 28.2 (1998): 137-144.
///
///\one problem about the formula is that when D is small, the computed
/// convective heat transfer
///  coefficient can be very large. This problem can be generally avoided in
///  CFD, as the size of first cell cannot be too small when a wall function is
///  deployed. However, in FFD, which usually does not use a wall function, this
///  could be a problem, as finer grid will be put near the boudary, to reoslve
///  the complexity of the flow in that region.
///
...
REAL h_coef(PARA_DATA *para, REAL **var, int i, int j, int k, REAL D) {
  ...
  default:
    sprintf(msg,
            "h_coef(): Value (%d) for para->prob->tur_model"
            "was not correct.",
            para->prob->tur_model);
    ffd_log(msg, FFD_ERROR);
  }
