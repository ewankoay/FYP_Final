...
int coef_diff(PARA_DATA *para, REAL **var, REAL *psi, REAL *psi0, int var_type,
  ...
  case C2:
    ...
    // non-uniform distribution over the space. Thus, when discretizing the
    // equation it is crtical to averge the thermal diffusivity at two
    // neibouring cells to calculate the value at the face.
    //
    // Failing to do this may result into the energy imbalance during the
    // diffusion term. To put in other words, one may observe that the heat flux
    // from the walls is not equal to the energy changing rate of the fluids.
    //
...
int source_diff(PARA_DATA *para, REAL **var, int var_type, int index) {
  ...
  case TEMP:
    b[IX(i, j, k)] +=
        var[TEMPS][IX(i, j, k)] / (para->prob->rho * para->prob->Cp);
    break;
  case C1:
  case C2:
