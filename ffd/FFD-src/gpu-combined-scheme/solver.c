...
int FFD_solver(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  ***************************************************************************/
  next = 1;
  while (next == 1) {
    // check the average temperature of fluid to calculate the energy
    // conservation need to have a vector to store the temperature for
    // calculation of energy changing rate Special note: the average temperature
    // is NOT good for calculation of above item
    // if (para->solv->check_conservation) para->prob->Tem_Ave_LastTime =
    // average_volume(para, var, var[TEMP]);
    ...
    if (para->mytime->t > t_steady && para->outp->cal_mean == 0) {
      ...
      if (flag != 0) {
        ffd_log("FFD_solver(): Could not reset averaged data.", FFD_ERROR);
        return flag;
      }
      else
    ...
    if (para->outp->cal_mean == 1) {
      ...
      if (flag != 0) {
        ffd_log("FFD_solver(): Could not add the averaged data.", FFD_ERROR);
        return 1;
      }
    }
...
int temp_step(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  // if (para->solv->check_conservation) para->prob->Tem_Ave_LastTime =
  // average_volume(para, var, var[TMP1]);

  // check residual after iterative solver
  if (para->solv->advection_solver == UPWIND &&
      para->solv->check_residual == 1) {
    residual = check_residual(para, var, T0, var[FLAGP]);
    // printf("residual after advection is: %f\n", residual);
    ...
    sprintf(msg, "Energy imbalance (W) after using implicit scheme: %f",
            para->prob->Energy_Imb_Adv);
    ffd_log(msg, FFD_NORMAL);
  }
...
int vel_step(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  if (para->solv->tile_flow_correct == PRESSURE_BASE)
    flag = tile_pressure_correction_method(para, var, BINDEX);
  ...
  if (flag != 0) {
    ffd_log("vel_step(): Could not determine the velocity for the tiles in "
            "pure tile modeling.",
            FFD_ERROR);
    return flag;
  }
  ...
  if (para->bc->nb_rack != 0) {
    if (rack_model_black_box(para, var, BINDEX) != 0) {
      ffd_log("tile_room_coupled: can not execute the black box model",
              FFD_ERROR);
    }
  }
  ...
  // forced mass conservation function is NOT on after projection is
  // Pressure-based correction is applied to tiles.
  // if(para->bc->nb_outlet!=0 && para->solv->mass_conservation_on ==1) flag =
  // mass_conservation(para, var,BINDEX);
  if (flag != 0) {
...
///////////////////////////////////////////////////////////////////////////////
int equ_solver(PARA_DATA *para, REAL **var, FFD_TERM which_term, int var_type,
               REAL *psi) {
  ...
  // now FFD can allow setting different number of iterations for each term,
  // separately However, users cannot set different number of iterations for
  // same terms for solving different variables. Users can set different number
  // of swipes for GS or Jacobi in input file, input.ffd Wei Tian, 06/23/17
  /************************************************************************************************/
  if (which_term == ADV) {
    // advection
...
int CheckImbalance(PARA_DATA *para, REAL **var, int var_type, int **BINDEX) {
  ...
  // rate_of_change = rho_cp*V*(T_ave - para->prob->Tem_Ave_LastTime) /
  // para->mytime->dt;

  // sum up of the energy changing rate of each cell

  FOR_EACH_CELL
  if (var[FLAGP][IX(i, j, k)] >= 0)
    continue;
  V = vol(para, var, i, j, k);
  T_ave = var[TEMP][IX(i, j, k)] - var[TMP1][IX(i, j, k)];
  rate_of_change +=
      para->prob->rho * para->prob->Cp * T_ave * V / para->mytime->dt;
  END_FOR
  ...
  if (para->mytime->step_current == 1) {
    ...
    fprintf(FILE_IM, "Time\t\tInflow\t\tOutflow\t\tWall\t\tDeficit\t\tEnergyRat"
                     "e\t\tAdvEnergy\t\tImbalance (Ein+Ewall-Eout)/Ewall\n");
    fprintf(FILE_IM, "%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n", para->mytime->t, in,
            out, wall, (in - out + wall), rate_of_change,
...
/// first implementation, @ Schneider Electric, Andover, MA
///
///\param para Pointer to FFD parameters
///\param var Pointer to FFD simulation variables
///\param BINDEX Pointer to boundary index
...
///\return 0 if no error occurred
///
/// 6/20/2017 Note: the whole idea is about the energy conservation which states
/// that the energy injected into the room should be energy gain in the fluid
/// when calculate the energy injection from the inlet and outlet, note that a
/// "dt" must be applied as in this function, the unit is J, instead of W.
///
/// The whole comments are made under context of energy. Surely, the codes also
/// work for other scalar variables, like specie concentration.
///
/// the density of the air is assumed to be one.
///
/// The equation of 14 and 15 in the paper is incorrect. The denominator should
/// essentially be the sum up of the numerator.
//////////////////////////////////////////////////////////////////////////////////////
int scalar_conservation(PARA_DATA *para, REAL **var, REAL *psi0, REAL *psi,
  ...
  int kmax = para->geom->kmax;
  int IMAX = imax + 2, IJMAX = (imax + 2) * (jmax + 2);
  REAL *gx = var[GX], *gy = var[GY],
       *gz = var[GZ];          // the surface coordinates of the cell
  ...
  REAL ita =
      0.0;  // the coefficient to distribute the global deficit into each cell
  REAL denominator = 0.0;  // the denominator to calculate the ita
  REAL *local_max = var[LOCMAX],
       *local_min = var[LOCMIN];  // store local maximum or minimum
  REAL *flagp = var[FLAGP];       // flag for cells
  int depX = 0, depY = 0,
      depZ = 0;  // the departure point dimensions for point (i,j,k)

  // calculate the deficit or suplus of the variable
  surplus = 0.0;
  /*1 Inflow scalar before advection*/
  surplus += inflow(para, var, psi0, BINDEX) * Cp *
             rho;  // without multiplying a dt, the unit is W
  /*2 outflow scalar before advection*/
  surplus -= outflow(para, var, psi0, BINDEX) * Cp *
             rho;  // without multiplying a dt, the unit is W
  surplus *=
      para->mytime->dt;  // multiple a time step size and W converts into J.
  /*3 Embody scalar before advection*/
  /*4 Embody scalar after advection*/
  FOR_EACH_CELL
  if (flagp[IX(i, j, k)] >= 0)
    continue;
  V_cell = (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
  ...
  // calculate the maximum and minimum of the original and departure value as
  // denominator for ita
  ...
  if (surplus > 0) {
    ...
    psi[IX(i, j, k)] +=
        surplus * (tmp / denominator /*ita*/) / (rho * Cp * V_cell);
    END_FOR
  }
  else {
    FOR_ALL_CELL
...
REAL adv_inconservation(PARA_DATA *para, REAL **var, REAL *psi0, REAL *psi,
  ...
  surplus = 0.0;
  /*1 Inflow scalar before advection*/
  surplus += inflow(para, var, psi0, BINDEX) *
             Cp;  // without multiplying a dt, the unit is W
                  /*2 outflow scalar before advection*/
  surplus -= outflow(para, var, psi0, BINDEX) *
             Cp;  // without multiplying a dt, the unit is W
  surplus *=
      para->mytime->dt;  // multiple a time step size and W converts into J.
  /*3 Embody scalar before advection*/
    ...
    if (flagp[IX(i, j, k)] == TILE) {
      if (i == imax + 1) {
        var[TILE_FLOW_BC][IX(i, j, k)] =
            pow(P_ave / var[TILE_RESI_BC][IX(i, j, k)], 0.5) / ayz;
      }
      if (i == 0) {
        var[TILE_FLOW_BC][IX(i, j, k)] =
            -1 * pow(P_ave / var[TILE_RESI_BC][IX(i, j, k)], 0.5) / ayz;
      }
      if (j == jmax + 1) {
        var[TILE_FLOW_BC][IX(i, j, k)] =
            pow(P_ave / var[TILE_RESI_BC][IX(i, j, k)], 0.5) / azx;
      }
      if (j == 0) {
        var[TILE_FLOW_BC][IX(i, j, k)] =
            -1 * pow(P_ave / var[TILE_RESI_BC][IX(i, j, k)], 0.5) / azx;
      }
      if (k == kmax + 1) {
        var[TILE_FLOW_BC][IX(i, j, k)] =
            pow(P_ave / var[TILE_RESI_BC][IX(i, j, k)], 0.5) / axy;
      }
      if (k == 0) {
        var[TILE_FLOW_BC][IX(i, j, k)] =
            -1 * pow(P_ave / var[TILE_RESI_BC][IX(i, j, k)], 0.5) / axy;
      }
      // printf("revised speed is--->>> %f\n", var[TILE_FLOW_BC][IX(i, j,
      // k)]);
    }
  }  // end of for
  return 0;
...
int tile_pressure_correction_method(PARA_DATA *para, REAL **var, int **BINDEX) {
  REAL p_prime_out = 0.0;
  REAL mass_imbalance_0 = 1.0, mass_imbalance_1 = 1.0, mass_imbalance_2 = 1.0;
  REAL p_prime_0 = 1.0, p_prime_1 = 1.0, p_prime_2 = 1.0;
  int INCREASE_P_GUESS = 1;
  int inner_count = 0;
  REAL tol = 1e-5;
  REAL p_prime_guess = 10;
  // bisection method
  // point 0, in which correction is 0
  p_prime_out = 0.0;
  mass_imbalance_0 =
      flowrate_pressure_correction(para, var, BINDEX, p_prime_out);
  if (fabs(mass_imbalance_0) <= tol) {
    para->bc->pressure_prime = p_prime_out;
    return 0;
  }
  p_prime_0 = p_prime_out;
  while (INCREASE_P_GUESS) {
    inner_count = 0;
    // Possibility1->point 1, in which pressure is p_prime_guess
    p_prime_1 = p_prime_guess;
    p_prime_out = p_prime_1;
    mass_imbalance_1 =
        flowrate_pressure_correction(para, var, BINDEX, p_prime_out);
    if (mass_imbalance_0 * mass_imbalance_1 < 0) {
      while (fabs(mass_imbalance_2) > tol) {
        // point 2, which is in the middle of point 0 and point 1
        // printf("mass imbalance is %f\n", (fabs(mass_imbalance_2)));
        // printf("p_prime postive is %f\n", p_prime_2);
        p_prime_2 = 0.5 * (p_prime_0 + p_prime_1);
        p_prime_out = p_prime_2;
        mass_imbalance_2 =
            flowrate_pressure_correction(para, var, BINDEX, p_prime_out);
        if (mass_imbalance_2 * mass_imbalance_0 < 0) {
          p_prime_1 = p_prime_2;
          mass_imbalance_1 = mass_imbalance_2;
        }
        else if (mass_imbalance_2 * mass_imbalance_1 < 0) {
          p_prime_0 = p_prime_2;
          mass_imbalance_0 = mass_imbalance_2;
        }
        else {
          para->bc->pressure_prime = p_prime_out;
          break;
    ...
    // Possibility2->point 1, in which pressure is p_prime_guess*-1
    inner_count = 0;
    p_prime_1 = p_prime_guess * -1;
    p_prime_out = p_prime_1;
    mass_imbalance_1 =
        flowrate_pressure_correction(para, var, BINDEX, p_prime_out);
    if (mass_imbalance_0 * mass_imbalance_1 < 0) {
      while (fabs(mass_imbalance_2) > tol) {
        // printf("p_prime negative is %f\n", p_prime_2);
        p_prime_2 = 0.5 * (p_prime_0 + p_prime_1);
        p_prime_out = p_prime_2;
        mass_imbalance_2 =
            flowrate_pressure_correction(para, var, BINDEX, p_prime_out);
    ...
    p_prime_guess = p_prime_guess * 5;
  }  // end of while (INCREASE_P_GUESS)
  // correct tile velocity
  update_tile_velocity(para, var, BINDEX);
  // update pressure
  update_tile_pressure(para, var, BINDEX);
...
///////////////////////////////////////////////////////////////////////////////////////
/// update the pressure for all cells
///\param para Pointer to FFD parameters
///\param var Pointer to FFD simulation variables
///\param BINDEX Pointer to boundary index
...
int update_tile_pressure(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  int kmax = para->geom->kmax;
  int IMAX = imax + 2, IJMAX = (imax + 2) * (jmax + 2);
  REAL p_prime = para->bc->pressure_prime;
...
int update_tile_velocity(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  int IMAX = imax + 2, IJMAX = (imax + 2) * (jmax + 2);
  int it;
  int bc_index = para->geom->index;
  REAL axy, ayz, azx, A;
  REAL *flagp = var[FLAGP];
  REAL *p = var[IP];                     // pressure
  REAL *resistance = var[TILE_RESI_BC];  // 0.5f, and alpha = 0.5f/A^2
  REAL alpha;                            // deltaP = alpha*Q^2
  REAL p_total;
  REAL p_prime = para->bc->pressure_prime;
  // update the flow rates at the tiles
  for (it = 0; it < bc_index; it++) {
    i = BINDEX[0][it];
    j = BINDEX[1][it];
    k = BINDEX[2][it];
    if (flagp[IX(i, j, k)] == TILE) {
      if (flagp[IX(i, j, k)] == TILE) {
        ...
        // calculate alpha
        if (A > SMALL) {
          alpha = resistance[IX(i, j, k)] / (A * A);
        }
        else {
          printf("ERROR: the area is zero\n");
        }
        // calculate total pressure
        p_total = p[IX(i, j, k)] + p_prime;
        // calculate velocity at each tile cell
        var[TILE_FLOW_BC][IX(i, j, k)] =
            sign(p_total) * sqrt(1 / alpha * fabs(p_total)) / A;
      }
    }
  }  // end of for
  return 0;
...
REAL flowrate_pressure_correction(PARA_DATA *para, REAL **var, int **BINDEX,
  ...
  REAL *p = var[IP];  // pressure
  REAL epsilon = 1e6;
  REAL flowrate_net = para->bc->flowrate_net;
  REAL axy = 0.0, ayz = 0.0, azx = 0.0, A = 0.0;
  REAL *resistance = var[TILE_RESI_BC];  // 0.5f, and alpha = 0.5f/A^2
  REAL alpha = 0.0;                      // deltaP = alpha*Q^2
  REAL p_total;
  // f= sum(sign(p_i+p_prime)*alpha*(p_i+p_prime)^(0.5)) -flowrate_ne
  REAL f = 0.0;  // total corrected flow rate
  ...
  for (it = 0; it < bc_index; it++) {
    ...
    if (flagp[IX(i, j, k)] == TILE) {
      ...
      p_total = p[IX(i, j, k)] + p_prime;
      // calculate f
      f += sign(p_total) * sqrt(1 / alpha * fabs(p_total));
    }
  }
  // mass imbalance
  epsilon = f - flowrate_net;
  // return
  return epsilon;
}
...
int tile_source(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  REAL *u = var[VX], *v = var[VY], *w = var[VZ];
  REAL axy, ayz, azx;
  int put_X = para->geom->tile_putX, put_Y = para->geom->tile_putY,
      put_Z = para->geom->tile_putZ;
      ...
      if (put_X) {
        if (u[IX(i, j, k)] > 0) {
          for (tile_index = 0; tile_index < split_limitX; tile_index++) {
            var[APXS][IX(i - tile_index - 2, j, k)] =
                2 * ayz * var[TILE_RESI_BC][IX(i, j, k)] *
                pow(u[IX(i, j, k)], 1) * 1.0 / (REAL)split_limitX;
            var[VXS][IX(i - tile_index - 2, j, k)] =
                1 * ayz * var[TILE_RESI_BC][IX(i, j, k)] *
                pow(u[IX(i, j, k)], 2) * 1.0 / (REAL)split_limitX;
          }
        }
        else {
          for (tile_index = 0; tile_index < split_limitX; tile_index++) {
            var[APXS][IX(i - tile_index - 2, j, k)] =
                ...
                pow(u[IX(i, j, k)], 1) * 1.0 / (REAL)split_limitX;
            var[VXS][IX(i - tile_index - 2, j, k)] =
                -ayz * var[TILE_RESI_BC][IX(i, j, k)] * pow(u[IX(i, j, k)], 2) *
                1.0 / (REAL)split_limitX;
          }
        }
      }
      else if (put_Y) {
        if (v[IX(i, j, k)] > 0) {
          for (tile_index = 0; tile_index < split_limitY; tile_index++) {
            var[APYS][IX(i, j - tile_index - 2, k)] =
                2 * azx * var[TILE_RESI_BC][IX(i, j, k)] *
                pow(v[IX(i, j, k)], 1) * 1.0 / (REAL)split_limitY;
            var[VYS][IX(i, j - tile_index - 2, k)] =
                1 * azx * var[TILE_RESI_BC][IX(i, j, k)] *
                pow(v[IX(i, j, k)], 2) * 1.0 / (REAL)split_limitY;
          }
        }
        else {
          for (tile_index = 0; tile_index < split_limitY; tile_index++) {
            var[APYS][IX(i, j - tile_index - 2, k)] =
                ...
                pow(v[IX(i, j, k)], 1) * 1.0 / (REAL)split_limitY;
            var[VYS][IX(i, j - tile_index - 2, k)] =
                -azx * var[TILE_RESI_BC][IX(i, j, k)] * pow(v[IX(i, j, k)], 2) *
                1.0 / (REAL)split_limitY;
          }
        }
      }
      else {
        if (w[IX(i, j, k - 1)] > 0) {
          for (tile_index = 0; tile_index < split_limitZ; tile_index++) {
            ...
            // var[APZS][IX(i, j, k-tile_index-2)] =
            // 2*axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)],
            // 1)*1.0/(REAL)split_limitZ; var[VZS][IX(i, j, k-tile_index-2)] =
            // 1*axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)],
            // 2)*1.0/(REAL)split_limitZ;
            /*Easy Linearization*/
            // var[APZS][IX(i, j, k-tile_index-2)] = axy*var[TILE_RESI_BC][IX(i,
            // j, k)] *pow(w[IX(i, j, k-1)], 1)*1.0/(REAL)split_limitZ;
            // var[VZS][IX(i, j, k-tile_index-2)] = 0;
            /*Lazy Method*/
            // var[VZS][IX(i, j, k-tile_index-2)] =
            // -1*axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)],
            // 2)*1.0/(REAL)split_limitZ; var[APZS][IX(i, j, k-tile_index-2)] =
            // 0;
            /*Semi-Implicit*/
            var[APZS][IX(i, j, k - tile_index - 2)] =
                2 * (1 - frac) * axy * var[TILE_RESI_BC][IX(i, j, k)] *
                pow(w[IX(i, j, k - 1)], 1) * 1.0 / (REAL)split_limitZ;
            var[VZS][IX(i, j, k - tile_index - 2)] =
                -1 * (2 * frac - 1) * axy * var[TILE_RESI_BC][IX(i, j, k)] *
                pow(w[IX(i, j, k - 1)], 2) * 1.0 / (REAL)split_limitZ;
          }
        }
        else {
          for (tile_index = 0; tile_index < split_limitZ; tile_index++) {
            ...
            // -2*axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)],
            // 1)*1.0/(REAL)split_limitZ; var[VZS][IX(i, j, k-tile_index-2)] =
            // -axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)],
            // 2)*1.0/(REAL)split_limitZ;
            /*Easy Linearization*/
            // var[APZS][IX(i, j, k-tile_index-2)] =
            // -1*axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)],
            ...
            // 0;
            /*Lazy Method*/
            // var[VZS][IX(i, j, k-tile_index-2)] = axy*var[TILE_RESI_BC][IX(i,
            // j, k)] *pow(w[IX(i, j, k-1)], 2)*1.0/(REAL)split_limitZ;
            // var[APZS][IX(i, j, k-tile_index-2)] = 0;
            /*Semi-Implicit*/
            var[APZS][IX(i, j, k - tile_index - 2)] =
                -2 * (1 - frac) * axy * var[TILE_RESI_BC][IX(i, j, k)] *
                pow(w[IX(i, j, k - 1)], 1) * 1.0 / (REAL)split_limitZ;
            var[VZS][IX(i, j, k - tile_index - 2)] =
                (2 * frac - 1) * axy * var[TILE_RESI_BC][IX(i, j, k)] *
                pow(w[IX(i, j, k - 1)], 2) * 1.0 / (REAL)split_limitZ;
          }
        }
      }
  ...
  // Output the tile flow rates information
  if (check_tile_flowrate(para, var, BINDEX) != 0) {
    ffd_log("tile_room_split: can not output the flow rates at tiles",
            FFD_ERROR);
  }
  return 0;
}
...
///
///\ Add a patch to fix the 0W rack situation by Wei Tian on 11/21/2018
///\ FIXME: need to parallelize this function in OpenCL to avoid unnecesary data
/// transfer
//////////////////////////////////////////////////////////////////////////////////////
    ...
    if (obj_type == RACK) {
      ...
      if (flagp[IX(i, j, k)] == RACK_INLET) {
        if (para->bc->RackDir[id] == 1 || para->bc->RackDir[id] == -1) {
          if (para->bc->RackFlowRate[id] < SMALL)
            var[VXBC][IX(i, j, k)] = SMALL * sign(para->bc->RackDir[id]);
          else
            var[VXBC][IX(i, j, k)] = para->bc->RackFlowRate[id] /
                                     para->bc->RackArea[id] *
                                     sign(para->bc->RackDir[id]) /*direction*/;
          var[VYBC][IX(i, j, k)] = 0.0;
          var[VZBC][IX(i, j, k)] = 0.0;
          // Assign the adjacent fluid cell temperature to the inlet of rack
          var[TEMPBC][IX(i, j, k)] =
              var[TEMP][IX(i - sign(para->bc->RackDir[id]), j, k)];
        }
        else if (para->bc->RackDir[id] == 2 || para->bc->RackDir[id] == -2) {
          if (para->bc->RackFlowRate[id] < SMALL)
            var[VYBC][IX(i, j, k)] = SMALL * sign(para->bc->RackDir[id]);
          else
            var[VYBC][IX(i, j, k)] = para->bc->RackFlowRate[id] /
                                     para->bc->RackArea[id] *
                                     sign(para->bc->RackDir[id]) /*direction*/;
          ...
          // Assign the adjacent fluid cell temperature to the inlet of rack
          var[TEMPBC][IX(i, j, k)] =
              var[TEMP][IX(i, j - sign(para->bc->RackDir[id]), k)];
        }
        else if (para->bc->RackDir[id] == 3 || para->bc->RackDir[id] == -3) {
          if (para->bc->RackFlowRate[id] < SMALL)
            var[VZBC][IX(i, j, k)] = SMALL * sign(para->bc->RackDir[id]);
          else
            var[VZBC][IX(i, j, k)] = para->bc->RackFlowRate[id] /
                                     para->bc->RackArea[id] *
                                     sign(para->bc->RackDir[id]) /*direction*/;
          var[VXBC][IX(i, j, k)] = 0.0;
          var[VYBC][IX(i, j, k)] = 0.0;
          // Assign the adjacent fluid cell temperature to the inlet of rack
          var[TEMPBC][IX(i, j, k)] =
              var[TEMP][IX(i, j, k - sign(para->bc->RackDir[id]))];
        }
        else {
          ffd_log("rack_model_black_box(): fail to detect the flow direction "
                  "of the rack",
                  FFD_ERROR);
          return 1;
        }
      ...
      // Assign velocity and temperature to outlet of rack
        ...
        if (para->bc->RackDir[id] == 1 || para->bc->RackDir[id] == -1) {
          ...
          Q_dot = para->bc->HeatDiss[id] * ayz /
                  para->bc->RackArea[id];  // heat dissipation by area
          mDot_Cp = para->prob->rho * var[VXBC][IX(i, j, k)] * ayz *
                    para->prob->Cp;  // mass flow rate multiply Cp
          var[TEMPBC][IX(i, j, k)] =
              var[TEMPBC][IX(iin, jin, kin)] +
              sign(para->bc->RackDir[id]) * Q_dot / mDot_Cp;
          // printf("rack_model_black_box(): temperature at outlet of rack [%d,
          // %d, %d]: %f\n",i,j,k,var[TEMPBC][IX(i,j,k)]);
          // printf("rack_model_black_box(): velocity at outlet of rack [%d, %d,
          ...
          jin = j - sign(para->bc->RackDir[id]) * para->bc->RackMap[id][1];
          kin = k - sign(para->bc->RackDir[id]) * para->bc->RackMap[id][2];
          Q_dot = para->bc->HeatDiss[id] * azx /
                  para->bc->RackArea[id];  // heat dissipation by area
          mDot_Cp = para->prob->rho * var[VYBC][IX(i, j, k)] * azx *
                    para->prob->Cp;  // mass flow rate multiply Cp
          var[TEMPBC][IX(i, j, k)] =
              var[TEMPBC][IX(iin, jin, kin)] + Q_dot / mDot_Cp;
        }
        else if (para->bc->RackDir[id] == 3 || para->bc->RackDir[id] == -3) {
          if (para->bc->RackFlowRate[id] < SMALL)
            var[VZBC][IX(i, j, k)] = SMALL * sign(para->bc->RackDir[id]);
          ...
          jin = j - sign(para->bc->RackDir[id]) * para->bc->RackMap[id][1];
          kin = k - sign(para->bc->RackDir[id]) * para->bc->RackMap[id][2];
          Q_dot = para->bc->HeatDiss[id] * axy /
                  para->bc->RackArea[id];  // heat dissipation by area
          mDot_Cp = para->prob->rho * var[VZBC][IX(i, j, k)] * axy *
                    para->prob->Cp;  // mass flow rate multiply Cp
          var[TEMPBC][IX(i, j, k)] =
              var[TEMPBC][IX(iin, jin, kin)] + Q_dot / mDot_Cp;
        }
