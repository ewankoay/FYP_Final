...
REAL vol(PARA_DATA *para, REAL **var, int i, int j, int k) {
  ...
  return area_xy(para, var, i, j, k) * length_z(para, var, i, j, k);
}  // End of vol()
...
REAL area_xy(PARA_DATA *para, REAL **var, int i, int j, int k) {
  return length_x(para, var, i, j, k) * length_y(para, var, i, j, k);
}  // End of area_xy()
...
REAL area_yz(PARA_DATA *para, REAL **var, int i, int j, int k) {
  return length_y(para, var, i, j, k) * length_z(para, var, i, j, k);
}  // End of area_yz();
...
REAL area_zx(PARA_DATA *para, REAL **var, int i, int j, int k) {
  return length_z(para, var, i, j, k) * length_x(para, var, i, j, k);
}  // End of area_zx()
...
int bounary_area(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  if (para->bc->nb_port > 0)
    for (id = 0; id < para->bc->nb_port; id++)
      APort[id] = 0;
  ...
  for (it = 0; it < index; it++) {
    ...
    if (obj_type == RACK && flagp[IX(i, j, k)] == RACK_INLET) {
      if (para->bc->RackDir[id] == 1 || para->bc->RackDir[id] == -1) {
        ...
        ffd_log(
            "rack_fluid_area(): fail to detect the flow direction of the rack",
            FFD_ERROR);
        return 1;
      }
