...
///
/// \date   7/05/2017
/// \add: add a function min_distance to calculate the distance of a fluid cell
/// to the nearest solid boundary condition, which is to be used by Chen's zero
...
void ffd_monitor_log(PARA_DATA *para, REAL **var, FFD_MSG_TYPE msg_type) {
  ...
  int kmax = para->geom->kmax;
  int IMAX = imax + 2, IJMAX = (imax + 2) * (jmax + 2);
  int monitorX, monitorY, monitorZ;

  //monitorX = round(imax / 2.0);
  //monitorY = round(jmax / 2.0);
  //monitorZ = round(kmax / 2.0);
		monitorX = 15.0;
		monitorY = 11.0;
		monitorZ = 2.4;

  if (msg_type == FFD_NEW) {
    if ((file_log = fopen("monitot_point_log.ffd", "w+")) == NULL) {
      fprintf(stderr, "Error:can not open error file!\n");
      exit(1);
    }
  }
    ...
    if ((file_log = fopen("monitot_point_log.ffd", "a+")) == NULL) {
      fprintf(stderr, "Error:can not open error file!\n");
      exit(1);
    }
    else {
      sprintf(msg, "%.6f\t%.6f\t%.6f\t%.6f\t%.6f", para->mytime->t,
              var[VX][IX(monitorX, monitorY, monitorZ)],
              var[VY][IX(monitorX, monitorY, monitorZ)],
              var[VZ][IX(monitorX, monitorY, monitorZ)],
              var[TEMP][IX(monitorX, monitorY, monitorZ)]);
    ...
    if (flagp[IX(i, j, k)] == 2 || flagp[IX(i, j, k)] == TILE) {
      ...
      if (i == imax + 1)
        mass_out += psi[IX(i - 1, j, k)] * u[IX(i - 1, j, k)] *
                    ...
                    (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      if (j == 0)
        mass_out += psi[IX(i, j, k)] * (-v[IX(i, j, k)]) *
                    (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                    (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      if (j == jmax + 1)
        mass_out += psi[IX(i, j, k)] * v[IX(i, j - 1, k)] *
                    (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                    (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      if (k == 0)
        mass_out += psi[IX(i, j, k)] * (-w[IX(i, j, k)]) *
                    (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                    (gy[IX(i, j, k)] - gy[IX(i, j - 1, k)]);
      if (k == kmax + 1)
        mass_out += psi[IX(i, j, k)] * w[IX(i, j, k - 1)] *
                    (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                    (gy[IX(i, j, k)] - gy[IX(i, j - 1, k)]);
    }
  }
...
REAL inflow(PARA_DATA *para, REAL **var, REAL *psi, int **BINDEX) {
  ...
  for (it = 0; it < index; it++) {
    ...
    if (flagp[IX(i, j, k)] == 0) {
      if (i == 0)
        mass_in += psi[IX(i, j, k)] * u[IX(i, j, k)] *
                   (gy[IX(i, j, k)] - gy[IX(i, j - 1, k)]) *
                   (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      if (i == imax + 1)
        mass_in += psi[IX(i, j, k)] * (-u[IX(i, j, k)]) *
                   ...
                   (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      if (j == 0)
        mass_in += psi[IX(i, j, k)] * v[IX(i, j, k)] *
                   (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                   (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      if (j == jmax + 1)
        mass_in += psi[IX(i, j, k)] * (-v[IX(i, j, k)]) *
                   ...
                   (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      if (k == 0)
        mass_in += psi[IX(i, j, k)] * w[IX(i, j, k)] *
                   (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                   (gy[IX(i, j, k)] - gy[IX(i, j - 1, k)]);
      if (k == kmax + 1)
        mass_in += psi[IX(i, j, k)] * (-w[IX(i, j, k)]) *
...
REAL check_min(PARA_DATA *para, REAL *psi, int ci, int cj, int ck) {
  ...
  for (i = 0; i <= 1; i++)
    for (j = 0; j <= 1; j++)
      for (k = 0; k <= 1; k++) {
        if (tmp > psi[IX(ci + i, cj + j, ck + k)])
          tmp = psi[IX(ci + i, cj + j, ck + k)];
      }
...
int min_distance(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  for (it = 0; it < index; it++) {
    ...
    j_bc = BINDEX[1][it];
    k_bc = BINDEX[2][it];
    if (var[FLAGP][IX(i_bc, j_bc, k_bc)] == INLET ||
        var[FLAGP][IX(i_bc, j_bc, k_bc)] == OUTLET)
      continue;
...
REAL vol_inflow(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  for (it = 0; it < index; it++) {
    ...
    if (flagp[IX(i, j, k)] == 0) {
      if (i == 0)
        mass_in += u[IX(i, j, k)] * (gy[IX(i, j, k)] - gy[IX(i, j - 1, k)]) *
                   (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      ...
      if (j == 0)
        mass_in += v[IX(i, j, k)] * (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                   (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      ...
      if (k == 0)
        mass_in += w[IX(i, j, k)] * (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                   (gy[IX(i, j, k)] - gy[IX(i, j - 1, k)]);
  ...
  for (it = 0; it < index; it++) {
    ...
    if (flagp[IX(i, j, k)] == OUTLET) {
      if (i == 0) {
        ...
        mass_out += u[IX(i, j, k)] * (gy[IX(i, j, k)] - gy[IX(i, j - 1, k)]) *
                    (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      }
      ...
      if (j == 0) {
        ...
        mass_out += v[IX(i, j - 1, k)] *
                    (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                    (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      }
      else {
        mass_out += v[IX(i, j, k)] * (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                    (gz[IX(i, j, k)] - gz[IX(i, j, k - 1)]);
      }
      ...
      if (k == 0) {
        ...
        mass_out += w[IX(i, j, k - 1)] *
                    (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                    (gy[IX(i, j, k)] - gy[IX(i, j - 1, k)]);
      }
      else {
        mass_out += w[IX(i, j, k)] * (gx[IX(i, j, k)] - gx[IX(i - 1, j, k)]) *
                    (gy[IX(i, j, k)] - gy[IX(i, j - 1, k)]);
      }
    }
  }
...
int get_monitor_points(PARA_DATA *para, REAL **var, int SI, int SJ, int SK,
  ...
  REAL off_set = 0.1524 * 0.5;  // 3 inch from the rack
  ...
  if (sign(rack_flow_direction) == 1) {
    if (SI == 0) {
      ffd_log("get_monitor_points(): monitor points are ouside the domain",
              FFD_ERROR);
      return 1;
    }
    else {
      monitorX = var[GX][IX(SI - 1, SJ, SK)] - off_set;
    }
  }
  else if (sign(rack_flow_direction) == -1) {
    monitorX = var[GX][IX(EI, SJ, SK)] + off_set;
  }
  else {
    ffd_log(
        "get_monitor_points(): ONLY ABLE TO HANDLE RACK FLOW DIRECTION OF X",
        FFD_ERROR);
    return 1;
  }

  // manually set the height of the monitor points based on the paper
  // Zachary Pardey, Jim VanGilder, 2015, CREATING A CALIBRATED CFD MODEL OF A
  // MIDSIZE DATA CENTER P1: 0.53m, P2: 0.91m, P3: 1.30m, P4: 1.68m in Z
  // direction para->sens->coordMoniPoints[A][B][C]-> A: which rack; B: which
  // monitor point; C: which coordinate
  for (i = 0; i < 4; i++) {
  ...
  for (id = 0; id < para->bc->nb_rack; id++) {
    for (point = 0; point < 4; point++) {
      ...
      if (flagp[IX(i, j, k)] != FLUID)
        continue;
      eulerian_dis =
          sqrt((xp - var[X][IX(i, j, k)]) * (xp - var[X][IX(i, j, k)]) +
...
int write_monitor_data(PARA_DATA *para, REAL **var) {
  ...
  if (get_monitor_index(para, var) != 0) {
    ffd_log("write_monitor_data(): cannot find the index of monitoring cells",
            FFD_ERROR);
  }
  // open output file
...
/// return 0 if all points are found
///////////////////////////////////////////////////////////////////////////////
int write_monitor_data_speed(PARA_DATA *para, REAL **var) {
  int i, j;
  int ip, jp, kp;
  ...
  // open output file
  if ((datafile = fopen("monitor_data_speed.dat", "w+")) == NULL) {
    ffd_log("write_monitor_data(): Failed to open output file!\n", FFD_ERROR);
    return 1;
  }
  // write header
  ...
  for (i = 0; i < para->bc->nb_rack; i++) {
    ...
    jp = para->sens->indexMoniPoints[i][0][1];
    kp = para->sens->indexMoniPoints[i][0][2];
    uo = var[VXM][IX(ip, jp, kp)];
    vo = var[VYM][IX(ip, jp, kp)];
    wo = var[VZM][IX(ip, jp, kp)];
    sp1 = sqrt(uo * uo + vo * vo + wo * wo);
    // printf("ID:%d,U:%f,V:%f,W:%f\n",i,uo,vo,wo);
    // P2
    ip = para->sens->indexMoniPoints[i][1][0];
    jp = para->sens->indexMoniPoints[i][1][1];
    kp = para->sens->indexMoniPoints[i][1][2];
