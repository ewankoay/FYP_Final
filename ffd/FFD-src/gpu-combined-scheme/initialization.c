...
///					a fluid cell to the nearest solid
/// boundary
/// condition, which 					is to be used by Chen's
...
void set_default_parameter(PARA_DATA *para) {
  ...
  para->mytime->t_steady =
      0.0;  // FIXME: assume the start of simulation as steady state
  para->mytime->t_start = clock();

  // Physical Properties
  para->prob->nu = (REAL)2.376e-5;
  ...
  para->prob->gravy = -9.8;
  para->prob->gravz = 0.0;
  para->prob->chen_a = (REAL)0.03874;  // Coefficient of Chen's model
  ...
  // Solving settings
  para->solv->check_residual = 1;        // Donot check residual
  para->solv->solver = GS;               // Gauss-Seidel Solver
  para->solv->interpolation = BILINEAR;  // Bilinear interpolation
  para->solv->SOR_U = 0.5;               // under-relaxation for U velocity
  para->solv->SOR_V = 0.5;               // under-relaxation for V velocity
  para->solv->SOR_W = 0.5;               // under-relaxation for W velocity
  para->solv->num_inner_iteration = 1;   // only one inner iteration
  ...
  para->inpu->read_old_ffd_file =
      0;  // Do not read the old FFD data as initial value
  sprintf(para->inpu->parameter_file_name, "%s", "input.cfd");
  ...
  para->outp->winz = 600;
  para->outp->v_ref = 1.0;
  para->outp->version = RUN;  // Running the debug version
  para->outp->i_N = 1;
  para->outp->j_N = 1;
  para->outp->k_N = 1;
  ...
  para->outp->mouse_j = 0;
  para->outp->mouse_k = 0;
  para->outp->result_file = STDOUT;  // set writing results to standard out
  ...
  para->bc->outlet_bc = PRESCRIBED_VALUE; //PRESCRIBED_VALUE;  // assume Neumann BC for outlet ZERO_GRADIENT
  para->bc->flowrate_in = 0;               // zero inflow rate
  para->bc->flowrate_out = 0;              // zero outflow rate
  para->bc->flowrate_net = 0;              // inflow rate - outflow rate
  para->bc->pressure_prime = 0;            // pressure correction is zero
...
int set_initial_data(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  ****************************************************************************/
  ...
  // FIXME: the alpha in the input.cfd is overwritten by k/(rho*cp).
  ...
  ****************************************************************************/
  if (para->sens->nb_sensor > 0) {
    ...
    para->sens->senValMean =
        (REAL *)malloc(para->sens->nb_sensor * sizeof(REAL));
  ...
  if (para->bc->nb_port > 0 && para->bc->nb_Xi > 0) {
    ...
    for (i = 0; i < para->bc->nb_port; i++) {
      ...
      if (para->bc->XiPort[i] == NULL || para->bc->XiPortAve[i] == NULL ||
        ...
        sprintf(msg,
                "set_initial_data(): Could not allocate memory for "
                "Xi at Port[%d].",
                i);
        ffd_log(msg, FFD_ERROR);
        return 1;
  ...
  ****************************************************************************/
  ...
  // calculate the minimal distance of a fluid cell to solid boudaries
  ...
  if (para->prob->tur_model == CHEN) {
    if (min_distance(para, var, BINDEX) != 0) {
                ffd_log("set_initial_data():  Could not find minimal value for
  zero equation model.", FFD_ERROR);
    }
  }
  */
  ...
  if (para->bc->nb_rack != 0) {
    if (rack_fluid_area(para, var, BINDEX) != 0) {
      ffd_log("set_initial_data():  Could not calculate the area of the rack "
              "inlet or outlet",
              FFD_ERROR);
    }
  }

  // find total supply flow rate
  set_bnd(para, var, VX, 0, var[VX], BINDEX);
  set_bnd(para, var, VY, 0, var[VY], BINDEX);
  set_bnd(para, var, VZ, 0, var[VZ], BINDEX);
  para->bc->flowrate_in = vol_inflow(para, var, BINDEX);
  para->bc->flowrate_out = vol_outflow(para, var, BINDEX);
...
int if_exist_tiles(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  if (num_tile > 0)
    ...
    ffd_log("if_exist_tiles(): can not figure out number of tile cells",
            FFD_ERROR);
    return -1;
  }
}
...
int init_para_simp(PARA_DATA *para, PARA_DATA_SIMP *para_simp) {
  ...
  para_simp->bc_simp.flowrate_in = para->bc->flowrate_in;
  para_simp->bc_simp.flowrate_out = para->bc->flowrate_out;
  para_simp->bc_simp.flowrate_net = para->bc->flowrate_net;
  para_simp->bc_simp.pressure_prime = para->bc->pressure_prime;
  para_simp->bc_simp.nb_rack = para->bc->nb_rack;
...
// prepare the data for rack
int get_rack_data(PARA_DATA *para, REAL **var, int **BINDEX, int *rack_dir,
                  REAL *rack_prop, int *map_matrix) {
  int it, index;
  int nb_rack = para->bc->nb_rack;
  // assign values for rack_prop
  for (it = 0; it < nb_rack; it++) {
    rack_dir[it] = para->bc->RackDir[it];
    rack_prop[RACK_FR * nb_rack + it] = para->bc->RackFlowRate[it];
    rack_prop[RACK_AREA * nb_rack + it] = para->bc->RackArea[it];
    rack_prop[RACK_HEAT * nb_rack + it] = para->bc->HeatDiss[it];
  }
  // assign values for rack map matrix
  for (it = 0; it < nb_rack; it++) {
    for (index = 0; index < 3; index++) {
      map_matrix[it * 3 + index] = para->bc->RackMap[it][index];
      // printf("mapping is %d\n", map_matrix[it * nb_rack + index]);
    }
  }
