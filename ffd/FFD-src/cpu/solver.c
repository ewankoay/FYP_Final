...
next = 1;
  while(next==1) {
    ...
    //if (para->solv->check_conservation) para->prob->Tem_Ave_LastTime = average_volume(para, var, var[TEMP]);
      ...
      if (flagp[IX(i, j, k)] == TILE) {
        ...
        if (w[IX(i, j, k-1)] > 0) {
          for (tile_index=0; tile_index<split_limitZ; tile_index++) {
            ...
            //var[APZS][IX(i, j, k-tile_index-2)] = 2*axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)], 1)*1.0/(REAL)split_limitZ;
            //var[VZS][IX(i, j, k-tile_index-2)] = 1*axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)], 2)*1.0/(REAL)split_limitZ;
            /*Easy Linearization*/
            //var[APZS][IX(i, j, k-tile_index-2)] = axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)], 1)*1.0/(REAL)split_limitZ;
          ...
          for (tile_index=0; tile_index<split_limitZ; tile_index++) {
            /*Linearization using Taylor Expansion*/
            //var[APZS][IX(i, j, k-tile_index-2)] = -2*axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)], 1)*1.0/(REAL)split_limitZ;
            //var[VZS][IX(i, j, k-tile_index-2)] = -axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)], 2)*1.0/(REAL)split_limitZ;
            /*Easy Linearization*/
            //var[APZS][IX(i, j, k-tile_index-2)] = -1*axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)], 1)*1.0/(REAL)split_limitZ;
            //var[VZS][IX(i, j, k-tile_index-2)] = 0;
            /*Lazy Method*/
            //var[VZS][IX(i, j, k-tile_index-2)] = axy*var[TILE_RESI_BC][IX(i, j, k)] *pow(w[IX(i, j, k-1)], 2)*1.0/(REAL)split_limitZ;
            //var[APZS][IX(i, j, k-tile_index-2)] = 0;
