...
int advect(PARA_DATA *para, REAL **var, int var_type, int index,
  ...
  ****************************************************************************/
  ...
  case VX:
    ...
    if (flag != 0)
      ffd_log("advect(): Failed in advection for X-velocity.", FFD_ERROR);
    break;
  /*-------------------------------------------------------------------------
  ...
  case VY:
    ...
    if (flag != 0)
      ffd_log("advect(): Failed in advection for Y-velocity.", FFD_ERROR);
    break;
  /*-------------------------------------------------------------------------
  ...
  case VZ:
    ...
    if (flag != 0)
      ffd_log("advect(): Failed in advection for Z-velocity.", FFD_ERROR);
    break;
  /*-------------------------------------------------------------------------
  ...
  case C2:
    ...
    if (flag != 0) {
      sprintf(msg, "advect(): Failed in advection for scalar variable type %d.",
              var_type);
  ...
  default:
    flag = 1;
    sprintf(msg,
            "advect(): Advection function not defined for variable "
            "type %d.",
            var_type);
    ffd_log(msg, FFD_ERROR);
  }
  ...
  ****************************************************************************/
  ...
  v0 = (REAL)0.5 *
       ((v[IX(i, j, k)] + v[IX(i, j - 1, k)]) *
            (x[IX(i + 1, j, k)] - gx[IX(i, j, k)]) +
        (v[IX(i + 1, j, k)] + v[IX(i + 1, j - 1, k)]) *
            (gx[IX(i, j, k)] - x[IX(i, j, k)])) /
       (x[IX(i + 1, j, k)] - x[IX(i, j, k)]);
  w0 = (REAL)0.5 *
       ((w[IX(i, j, k)] + w[IX(i, j, k - 1)]) *
            (x[IX(i + 1, j, k)] - gx[IX(i, j, k)]) +
        (w[IX(i + 1, j, k)] + w[IX(i + 1, j, k - 1)]) *
            (gx[IX(i, j, k)] - x[IX(i, j, k)])) /
       (x[IX(i + 1, j, k)] - x[IX(i, j, k)]);
...
int trace_vy(PARA_DATA *para, REAL **var, int var_type, REAL *d, REAL *d0,
  ...
  u0 = (REAL)0.5 *
       ((u[IX(i, j, k)] + u[IX(i - 1, j, k)]) *
            (y[IX(i, j + 1, k)] - gy[IX(i, j, k)]) +
        (u[IX(i, j + 1, k)] + u[IX(i - 1, j + 1, k)]) *
            (gy[IX(i, j, k)] - y[IX(i, j, k)])) /
       (y[IX(i, j + 1, k)] - y[IX(i, j, k)]);
  v0 = v[IX(i, j, k)];
  w0 = (REAL)0.5 *
       ((w[IX(i, j, k)] + w[IX(i, j, k - 1)]) *
            (y[IX(i, j + 1, k)] - gy[IX(i, j, k)]) +
        (w[IX(i, j + 1, k)] + w[IX(i, j + 1, k - 1)]) *
            (gy[IX(i, j, k)] - y[IX(i, j, k)])) /
       (y[IX(i, j + 1, k)] - y[IX(i, j, k)]);
  // Find the location at previous time step
...
int trace_vz(PARA_DATA *para, REAL **var, int var_type, REAL *d, REAL *d0,
  ...
  u0 = (REAL)0.5 *
       ((u[IX(i, j, k)] + u[IX(i - 1, j, k)]) *
            (z[IX(i, j, k + 1)] - gz[IX(i, j, k)]) +
        (u[IX(i, j, k + 1)] + u[IX(i - 1, j, k + 1)]) *
            (gz[IX(i, j, k)] - z[IX(i, j, k)])) /
       (z[IX(i, j, k + 1)] - z[IX(i, j, k)]);
  v0 = (REAL)0.5 *
       ((v[IX(i, j, k)] + v[IX(i, j - 1, k)]) *
            (z[IX(i, j, k + 1)] - gz[IX(i, j, k)]) +
        (v[IX(i, j, k + 1)] + v[IX(i, j - 1, k + 1)]) *
            (gz[IX(i, j, k)] - z[IX(i, j, k)])) /
       (z[IX(i, j, k + 1)] - z[IX(i, j, k)]);
  w0 = w[IX(i, j, k)];
  // Find the location at previous time step
...
int trace_scalar(PARA_DATA *para, REAL **var, int var_type, int index, REAL *d,
  ...
  while (COOD[X] == 1 || COOD[Y] == 1 || COOD[Z] == 1) {
    ...
    if (it > itmax) {
      sprintf(msg,
              "trace_scalar(): Could not track the location for scalar "
...
void set_x_location(PARA_DATA *para, REAL **var, REAL *flag, REAL *x, REAL u0,
                    int i, int j, int k, REAL *OL /*the coordinates*/,
                    int *OC /*the index*/, int *LOC /*hit wall*/,
                    int *COOD /*finish*/) {
  int imax = para->geom->imax, jmax = para->geom->jmax;
  int IMAX = imax + 2, IJMAX = (imax + 2) * (jmax + 2);
...
int coef_adv(PARA_DATA *para, REAL **var, REAL *psi, REAL *psi0, int var_type,
  ...
  REAL *af = var[AF], *ab = var[AB], *ap = var[AP], *ap0 = var[AP0],
       *b = var[B];
  REAL *x = var[X], *y = var[Y], *z = var[Z];
  ...
  case C2:
    ...
    // The solution is to change the denominator by removing Fe - Fw + Fn - Fs +
    // Ff - Fb in AP
    FOR_EACH_CELL
    ap[IX(i, j, k)] = ap0[IX(i, j, k)] + ae[IX(i, j, k)] + aw[IX(i, j, k)] +
                      an[IX(i, j, k)] + as[IX(i, j, k)] + af[IX(i, j, k)] +
...
int implicit_vx(PARA_DATA *para, REAL **var, int var_type, REAL *d, REAL *d0,
  ...
  if (equ_solver(para, var, ADV, var_type, d) != 0) {
    ffd_log("implicit_vx(): failed to solve the equation for advection",
            FFD_ERROR);
    return 1;
  }
  // Define B.C.
...
int implicit_vy(PARA_DATA *para, REAL **var, int var_type, REAL *d, REAL *d0,
  ...
  if (equ_solver(para, var, ADV, var_type, d) != 0) {
    ffd_log("implicit_vy(): failed to solve the equation for advection",
            FFD_ERROR);
    return 1;
  }
  // Define B.C.
...
int implicit_vz(PARA_DATA *para, REAL **var, int var_type, REAL *d, REAL *d0,
  ...
  if (equ_solver(para, var, ADV, var_type, d) != 0) {
    ffd_log("implicit_vz(): failed to solve the equation for advection",
            FFD_ERROR);
    return 1;
  }
  // Define B.C.
...
int implicit_scalar(PARA_DATA *para, REAL **var, int var_type, REAL *d,
  ...
  if (equ_solver(para, var, ADV, var_type, d) != 0) {
    ffd_log("implicit_scalar(): failed to solve the equation for advection",
            FFD_ERROR);
    return 1;
  }
  // Define B.C.
